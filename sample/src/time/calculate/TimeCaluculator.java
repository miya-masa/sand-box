...
public class TimeCaluculator {
	...
	public TimeStructure caluculateTime(long time) {
		TimeStructure structure = new TimeStructure();
		long diff = caluculateYear(structure, time);
		diff = calculateMonth(structure, diff, structure.getYear());
		diff = calculateDay(structure, diff, structure.getMonth(),
				structure.getYear());
		diff = calculateHour(structure, diff);
		diff = caluculateMinute(structure, diff);
		structure.setSecond(Long.valueOf(diff).intValue());
		return structure;
	}

	private long caluculateMinute(TimeStructure structure, long time) {
		int minute = 0;
		long diff = time;
	...
	private long calculateHour(TimeStructure structure, long time) {
		int hour = 0;
		long diff = time;
	...
	private long calculateDay(TimeStructure structure, long time, int month,
			int year) {
		long diff = time;
		int day = 1;
	...
	private long calculateMonth(TimeStructure structure, long time, int year) {
		long diff = time;
	...
	private long caluculateYear(TimeStructure structure, long time) {
